...
function PasswordRule({ ok, text }: { ok: boolean; text: string }) {
  return (
    <li className="flex items-center gap-1.5 text-xs">
      ...
      <span className={ok ? "text-emerald-400" : "text-white/35"}>{text}</span>
    </li>
  );
}
      ...
      >
        ...
        >
          <div className="text-center mb-8">
            <div className="text-2xl font-bold text-white mb-2" style={{ textShadow: "0 0 20px rgba(59,130,246,0.8)" }}>
              BruteScale
            </div>
            <h1 className="text-xl font-bold text-white mb-1">Create your account</h1>
            <p className="text-sm text-white/40">Start deploying in 60 seconds</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {password.length > 0 && (
              <ul className="space-y-1 pl-1">
                <PasswordRule ok={pwRules.length} text="At least 8 characters" />
                <PasswordRule ok={pwRules.match}  text="Passwords match" />
              </ul>
            )}
